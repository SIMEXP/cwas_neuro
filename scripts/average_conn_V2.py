...
def _load_parcels_compute_mask():
    ...
    # Build a group mask. Start by generating brain masks from the fMRI time series
    ...
    return img_mist, imgs, confounds, mask_combined

def _load_voxel_data(mask_combined, img_mist, img, confounds):
...
def _resample_mist(img_mist, mask_combined):
    ...
    return mist_resampled, mist_voxel

def _compute_tseries(mist_resampled, img_n):
...
def _compute_brute_afc(time_series_voxel, mist_voxel, masker_mist):
    ...
    for i in range(64):
        ...
        brute_afc_parcel = np.mean(conn_network[np.tril(np.ones(conn_network.shape), -1)==1])
...
def _extract_time_series_parcels(img, mask, parcels, confounds=None, smoothing_fwhm=None):
    ...
    time_series_voxel, masker_voxel = _extract_time_series_voxel(img, mask, confounds, smoothing_fwhm)
...
def _average_conn(img, mask, parcels, confounds=None, smoothing_fwhm=None):
    ...
    mask_empty = (size_parcels == 0) | (size_parcels == 1)
    test_afc = ((size_parcels * size_parcels) * var_parcels - size_parcels) / (size_parcels * (size_parcels - 1))
